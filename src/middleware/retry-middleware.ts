import { Action } from "@reduxjs/toolkit";
import { ThunkAction } from "redux-thunk";
import { Middleware } from "redux";

const isPromise = (value: any) => value !== null && typeof value === "function";

export type TRetryAction<A extends Action | ThunkAction<any, any, any, any>> = (
  dispatch: IRetryDispatch<A>
) => void;

export interface IRetryDispatch<
  BasicAction extends Action | ThunkAction<any, any, any, any>
> {
  (retryAction: BasicAction[]): void;

  <Action extends BasicAction>(action: Action): Action;

  <Action extends BasicAction>(
    action: BasicAction | BasicAction[]
  ): Action | void;
}

/// Если экшен это массив из двух функций, возвращающих промисы, выполняет первый промис
/// если не было ошибок заканчивает выполнение экшена
/// если были ошибки - пытается выполнить второй промис. В случае успеха поторно запускает обработку первого промиса
export function retryMiddleware() {
  const middleware: Middleware<
    IRetryDispatch<Action | ThunkAction<any, any, any, any>>,
    any,
    IRetryDispatch<Action | ThunkAction<any, any, any, any>>
  > = function middleware(store) {
    return function (next) {
      return async function (action) {
        if (
          Array.isArray(action) &&
          action.length === 2 &&
          isPromise(action[0]) &&
          isPromise(action[1])
        ) {
          store.dispatch(action[0]).catch((err: Error) => {
            console.log(`${err}`);
            store
              .dispatch(action[1])
              .then((res: any) =>
                store.dispatch(action[0]).catch((err: Error) => {
                  console.log(`${err}`);
                })
              )
              .catch((err: Error) => {
                console.log(`${err}`);
              });
          });
        } else {
          return next(action);
        }
      };
    };
  };
  return middleware;
}

const retry = retryMiddleware();
export default retry;
