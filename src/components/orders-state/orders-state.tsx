...
export const OrdersState: React.FC = () => {
  const message = useAppSelector(
    (store) => store.ws?.messages[store.ws?.messages.length - 1]
  );

  const doneOrders: IWsOrder[] = message?.orders?.filter((i) => {
    return i.status === "done";
  });
  const inWorkOrders: IWsOrder[] = message?.orders?.filter((i) => {
    return i.status !== "done";
  });
