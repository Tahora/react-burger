...
export function BurgerIngredients (props) {
    ...
    return (
        <section className={`${styles.burgerIngredients}`} >
            ...
            {props.burgerdata.filter((i)=>{return i.type==="bun"}).map((i,key)=>{
                const {name, price, image}=i;
                return (
                    <IngridientCard key={key} name={name} price={price} image={image} count={false} showModal={showModal(i)}/>
                )})}
            </div>
            <h2 className='text text_type_main-medium mt-10 mb-6'>Соусы</h2>
            <div className={`${styles.ingridientsTable} pl-4 pr-4`}>
                {props.burgerdata.filter((i)=>{return i.type==="sauce"}).map((i, key)=>{
                    const {name, price, image}=i;
                    return (
                        <IngridientCard key={key} name={name} price={price} image={image} count={false} showModal={showModal(i)}/>
                    )})}
            ...
            <div className={`${styles.ingridientsTable} pl-4 pr-4`}>
                {props.burgerdata.filter((i)=>{return i.type==="main"}).map((i, key)=>{
                    const {name, price, image}=i;
                    return (
                        <IngridientCard key={key} name={name} price={price} image={image} count={false} showModal={showModal(i)}/>
                    )})}
